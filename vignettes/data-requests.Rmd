...
A data request starts from a list of variables, provided in a `csv` or `xlsx` file in one of a variety of formats **[LINK]**. The first task is to read the variables file into an `ev_variables` **[LINK]** container, which contains all the information `epivaultr` needs to fetch the required data.
...
Now we no longer need the open connection to EpiVault, it is good practice to disconnect.
